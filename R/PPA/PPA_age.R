...
cutT        <- deltaT
maxT        <- 400 * deltaT # simulation terminal year
...
run_simulation <- function(spVitals_list, initComm)
{
    ...
    # Make species vital rates as objects in the global environment
    ...
    for (t in seq(0, maxT, by = deltaT))
    {
        ...
        # (4) species
        # (5) age
        mortality <- matrix(0, nrow = 0, ncol = 5)
        ...
        # Remove any cohort with too few individuals
        data     <- data[data[,2] > mincohortN, , drop = FALSE] 
        # Remove any cohort with a negative avg. diameter
        data     <- data[data[,1] > 0, , drop = FALSE]          

        # Step 3: Age each cohort
        data[,5] <- data[,5] + deltaT
        
        # Step 4: Reproduce
        # Step 4a. Internal seed rain
        ...
        # Step 4b. External seed rain
        if (CALCULATE_EXTERNAL_SEED_RAIN)
        {
            data <- rbind(data, externalSeedRain)
        }

        # Step 5: Assign crown class
...
{
    # Seed rain matrix
    tmp_externalSeedRain <- matrix(1, nrow = N, ncol = 5)
...
# removes any plants falling below the minimum considered canopy layer.
