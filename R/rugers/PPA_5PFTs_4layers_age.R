...
##########################################
run = function( mainfolder = paste0( getwd(), '/data/'),
	              vitalratesfile = "PPA_FG5_maxD.txt",
            	  initialconditionsfile = "PPA_initial_fg5_primary_age.txt",
            	  outputfile = "out_skratch_FG5.txt" ){
...
# initial conditions file - the initial conditions of the forest in the format: 
...
##########################################
	...
	# if(CA>PA){
	#   initdata          <- CCassign_manylayers_continuous(initdata, PA, deltaT) 
	# }
	# # kill plants that fall into a fifth layer immediately
	# initdata            <- initdata[initdata[,3]<5,]
	# names(initdata)     <- NULL; 
	# initdata            <- as.matrix(initdata)

	stats_l             <- main_cohorts(fgdata, initdata)
	
	write.table( stats_l[[1]], 
	             paste0(mainfolder, 'out_skratch_Slow.txt'), 
	             sep="\t", row.names=FALSE )
	
	write.table( stats_l[[1]], 
	             paste0(mainfolder, outputfile), 
	             sep="\t", row.names=FALSE )
	
	saveRDS( stats_l[[2]], 
           paste0(mainfolder, gsub('.txt','.rds',outputfile)) )
...
#####################
  ...
  # (4) functional group
  # (5) age
	ndata     <- matrix(1, nrow = dim(fgdata)[1], ncol = 5) 
	
	# initialize the matrix with initial cohort of individuals
	ndata[,1] <- dnot # initial minimum diameter
	ndata[,2] <- PA / 10000 * fgdata$F # recruits per-meter^2
	ndata[,3] <- 1 # if we start from a cohort, it's all in the canopy
	ndata[,4] <- fgdata$fg # functional groups
	
	# Produce a cohort every year (because time step is 5 yrs) and project growth/survival in the future
	baby1 = ndata; 
	baby1[,1] = baby1[,1] + fgdata$G4*4; 
	baby1[,2] = baby1[,2]*(1-fgdata$mu4)^4 ; 
	baby1[,5] = baby1[,5] + 4
	baby2 = ndata; 
	baby2[,1] = baby2[,1] + fgdata$G4*3; 
	baby2[,2] = baby2[,2]*(1-fgdata$mu4)^3; 
	baby2[,5] = baby2[,5] + 3
	baby3 = ndata; 
	baby3[,1] = baby3[,1] + fgdata$G4*2; 
	baby3[,2] = baby3[,2]*(1-fgdata$mu4)^2; 
	baby3[,5] = baby3[,5] + 2
	baby4 = ndata; 
	baby4[,1] = baby4[,1] + fgdata$G4*1; 
	baby4[,2] = baby4[,2]*(1-fgdata$mu4)^1; 
	baby4[,5] = baby4[,5] + 1
	baby5 = ndata
	
	babyMatrix      <- rbind(baby1, baby2, baby3, baby4, baby5)
	babyMatrix[,3]  <- 4 # IS THIS A MISTAKE?
	
	ndata           <- babyMatrix
	ndata[,3]       <- 1

	# only start from the "babies" if is.na(initdata[[1]]) 
	...
	for(t in seq(deltaT,maxT,by=deltaT) ){
	  ...
	  # functional groups loop
		for( fg in unique(data[,4]) ){
		  
			#Step 1: Mortality 
			muV   <- c( fgdata[fg,seq(7,10)], 1)
			
      # Kill individuals in each of the cohorts in a functional group
			for(i in seq(1,dim(data)[1])[data[,4]==fg]){
			  data[i,2] <- data[i,2]*(1-muV[[data[i,3]]])^deltaT 
			}
			
			# Only keep cohorts with enough individuals 
			data  <- data[data[,2]>mincohortN,,drop=FALSE]
			
			# Step 2: Growth by layer
			GV    <- fgdata[fg,seq(3,6)]
			
			# Grow individuals in each of the cohorts in a functional group
			for( layer in unique(data[data[,4]==fg,,drop=FALSE][,3]) ){
			  data[data[,4]==fg & data[,3]==layer,1] = data[data[,4]==fg&data[,3]==layer,1] + GV[[layer]]*deltaT
			}
			
		} # end functional groups loop
		...
		#Step 3: Reproduce
		#Note here, reproduction is not a function of this patch itself, but it easily could be with: 
		# babies = round(min(PA,sum(data[,1]^theta*phi*data[,2]))*Fnot*deltaT)
		...
		if(t>0) if(floor(t/cutT)==t/cutT){  #records data once every cutT timesteps.
			stats = rbind(stats,
			              data.frame(time=t,calcstats(data,fgdata)))
		}
		
		cohort_l[[t/5]] <- list( 'cohorts' = data )
		
	}
	
	list( 'stats'  = stats, 
	      'cohort' = cohort_l ) %>% return
...
######################################	
	...
	# reverse order: from small to big diameters
	data            <- data[order(data[,1]),]
  # number of cohorts
	out$numcohorts  <- dim(data)[1]
  # Only cohorts with diameter above 5 cm
	bigdata         <- data[data[,1]>=5,]
	
	# cycle through plant types AND
	# calculate total N. individuals, total basal area, and total aboveground biomass
	...
	for(fg in unique(data[,4])){
		...
		# separate dataset into diameter bins 
		# 1-5; 5-20; <20; 20-60; >60; >5 
		...
		# separate dataset into THREE diameter bins 
		# 5-20; 20-60; >60
	...
	# add generation time
	out = cbind( out, calc_gt( fgdata, data) )
...
######################################	
	...
	n       <- sum(godata[,2]) # sum all individuals
	ba      <- sum((godata[,1]/200)^2*pi*godata[,2]) # total basal area
...
######################################
	...
	for(layers in seq(1, (sum(cohortCAv)/PA)) ){
		...
		# pull out individuals (cohorts?) that are definitely in the canopy and understory
...
calc_gt <- function( fgdata, data ){
  
  # added calculations
  gr_df   <- dplyr::select( fgdata, fg, G1:G4 ) %>% 
    gather( layer, G, G1:G4 ) %>% 
    mutate( layer = gsub('G','',layer) ) %>% 
    mutate( layer = as.numeric(layer) ) 
  
  # back-calculate age from average growth rates
  data_df <- data %>% 
    as.data.frame %>% 
  ...
  # calculate maximum diameter at each time step
  max_df  <- data_df %>% 
    group_by( fg ) %>% 
    summarise( max_d = quantile(diam, probs=0.95) ) %>% 
    ungroup %>% 
    right_join( data.frame( fg = 1:5 ) ) %>% 
    pivot_wider( names_from  = fg,
                 values_from = max_d ) %>% 
    setNames( paste0('max_d_',names(.)) ) %>% 
    dplyr::select( paste0('max_d_', 1:5 ) )

  # generation time: only layer 1 reproduces (simplification)
  gt_c_df <- data_df %>% 
              subset( layer == 1 ) %>% 
              mutate( w = N * (phi*diam^theta) ) %>% 
              group_by( fg ) %>% 
              summarise( gt = weighted.mean(age, w) ) %>% 
              ungroup %>% 
              right_join( data.frame( fg = 1:5 ) ) %>% 
              pivot_wider( names_from  = fg,
                           values_from = gt ) %>% 
              setNames( paste0( 'gt_', names(.) ) ) %>% 
              dplyr::select( paste0('gt_', 1:5 ) )
             
  # generation time: only layer 1 reproduces; age-by-growth
  gt_c_g_df <- data_df %>% 
    subset( layer == 1 ) %>% 
    mutate( w = N * (phi*diam^theta) ) %>% 
    group_by( fg ) %>% 
    summarise( gt = weighted.mean(age_g, w) ) %>% 
    ungroup %>% 
    right_join( data.frame( fg = 1:5 ) ) %>% 
    pivot_wider( names_from  = fg,
                 values_from = gt ) %>% 
    setNames( paste0( 'gt_g_', names(.) ) ) %>% 
    dplyr::select( paste0('gt_g_', 1:5 ) )
  
  # generation time: 
  # all trees of half max. diameter reproduce
  gt_maxd_df <- dplyr::select( fgdata, fg, max_d ) %>% 
    left_join( data_df ) %>% 
    dplyr::select( diam, N, layer, fg, age, max_d ) %>% 
    mutate( half_d = max_d / 2 ) %>% 
    dplyr::select( -max_d ) %>% 
    subset( diam > half_d ) %>% 
    mutate( w = N * (phi*diam^theta) ) %>% 
    group_by( fg ) %>% 
    summarise( gt_diam = weighted.mean(age, w) ) %>% 
    ungroup %>% 
    right_join( data.frame( fg = 1:5 ) ) %>% 
    pivot_wider( names_from  = fg,
                 values_from = gt_diam ) %>% 
    setNames( paste0( 'gt_diam_', names(.) ) ) %>% 
    dplyr::select( paste0('gt_diam_', 1:5 ) )
  
  bind_cols( gt_c_df,  gt_c_g_df, gt_maxd_df, max_df )
