...
#####################
  ...
  # main matrix with columns: (1) cohort diameter (2) number of individuals (3) crown class (4) functional group
	ndata     <- matrix(1, nrow = dim(fgdata)[1], ncol = 4) 
	...
	ndata[,3] <- 1 # if we start from a cohort, it's all 
...
calc_gt <- function( fgdata, data ){
  ...
  # generation time: all trees of half max. diameter reproduce
  gt_maxd_df <- dplyr::select( fgdata, fg, max_d ) %>% 
    left_join( data_df ) %>% 
    dplyr::select( diam, N, layer, fg, G, age, max_d ) %>% 
  ...
  bind_cols( gt_c_df, gt_maxd_df, max_df )
